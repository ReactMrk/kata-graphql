...
import { GET_CUSTOMER, REMOVE_CUSTOMER } from "../queries/customer-queries";
...
const CustomerSearchAndRemove = () => {
    ...
    const [removeCustomerMutation] = useMutation(REMOVE_CUSTOMER);
    const [searchResultCustomer, setSearchResultCustomer] = useState();

    const searchCustomer = async () => {
        await getCustomer({
            variables: {email},
            onCompleted: data => setSearchResultCustomer(data.getCustomer)
        });
    };

    const removeCustomer = async () => {
        await removeCustomerMutation({
            variables: { email },
            onCompleted: data => {
                setCustomers(data.removeCustomer);
                setSearchResultCustomer(undefined);
            }
        });
    };
